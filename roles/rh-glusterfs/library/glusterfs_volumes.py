...
- name: create gluster volume
  glusterfs_volume: state=present name=test1 brick=/bricks/brick1/g1 rebalance=yes cluster:"{{ play_hosts }}"
  run_once: true

- name: tune
  glusterfs_volume: state=present name=test1 options='{performance.cache-size: 256MB}'

- name: start gluster volume
  glusterfs_volume: status=started name=test1

- name: limit usage
  glusterfs_volume: state=present name=test1 directory=/foo quota=20.0MB

- name: stop gluster volume
  glusterfs_volume: state=stopped name=test1

- name: remove gluster volume
  glusterfs_volume: state=absent name=test1
"""
...
def create_volume(name, stripe, replica, arbiter, transport, hosts, bricks, force):
    ...
    for brick in bricks:
        for host in hosts:
            args.append(('%s:%s/%s' % (host, brick, name)))
    if force:
        args.append('force')
    run_gluster(args)
        ...
        if volume_name in volumes:
            ...
            for node in cluster:
                for brick_path in brick_paths:
                    brick = '%s:%s/%s' % (node, brick_path, volume_name)
